...
│           └── 11_Articulos.py       ← catálogo bibliográfico filtrable + gestión DOIs (privada+pública)
├── deployment/
...
| `11_Articulos` | Privada + pública. Catálogo bibliográfico: resumen por categoría (`get_categories_summary`/`category_summary_row` en `app_utils.py`, con columna "Sin DOI") + listado desde `papers_metadata.jsonl` con filtros (texto título/DOI/autor, año, revista, radio Con/Sin DOI), DOI como `LinkColumn`, autores legibles (`_fmt_authors`) y export CSV de la vista filtrada. **Gestión de DOIs faltantes (solo privada):** editor de DOI con sugerencia Crossref (`crossref_suggest` con título+apellido1+año, `mailto=UNPAYWALL_EMAIL`); al guardar escribe en `papers_metadata.jsonl` (`.bak`) + upsert en `doi_manual.xlsx` (`assign_dois`). Commits 37d0e7c, b9e4eea, 9e95227. |
