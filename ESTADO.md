...
| Rollout del commit `6093f35` (2026-07-25) | `/Volumes/research/metadatos/eval/rollout_6093f35/informe.md` (NAS, fuera de git; mismo patrón que las dos anteriores — pendiente copiar a la raíz del repo desde pciq22 para versionarlo). Junto a él: `antes.json`/`despues.json`, `antes_REDERIVADO.json` y `logs_paso2/` | `Mejoras_pendientes.md` item **65** (vectores fantasma, nuevo) + correcciones de alcance en items 33/55a/55b/63 + nota en el 35 |
...
│   ├── run_eval.py                   ← evaluación Hit@k / MRR contra golden sets (item 37); --golden/--tag para goldens alternativos
...
│           └── 15_Pendientes.py      ← posponer / VETAR DOIs pendientes + purga dura del corpus (solo privada)
...
| `3_process_corpus.py` | PDF → TEI (GROBID) → MD clean → chunks JSONL. **2026-06-10 (item 32)**: campo `section_canonical` en cada chunk con herencia jerárquica de heading — `split_by_headings()` devuelve nivel del heading; `build_chunk_records()` mantiene mapa ancestros `nivel→canonical` y sube niveles hasta encontrar una etiqueta que no sea "other". 7 etiquetas: `abstract \| introduction \| methods \| results \| discussion \| conclusion \| other`. Chunks de tabla: `section_canonical="table"`. El campo `section` (título hoja crudo) queda intacto. Commit b097744. **2026-06-11**: chunks capados a `MAX_EMBED_CHARS` vía `_split_to_max_chars` (texto y TABLAS, que antes se emitían enteras); `section_part` añadido a registros de tabla. ~~**Defecto conocido (auditoría de chunks 2026-07-25, item 63):** `extract_tables_md` une filas de tabla con un solo `\n`; `_split_to_max_chars` solo corta en `\n\n`, así que una tabla ≥`MAX_EMBED_CHARS` cae al fallback por palabras sueltas y fusiona filas distintas en una sola entrada (confirmado en 2023_almenglo).~~ **✅ ARREGLADO 2026-07-25 (items 62 + 63), PENDIENTE DE ROLLOUT:** (63.1) filas de tabla como párrafos `\n\n` en AMBAS ramas de `extract_tables_md`, que ahora devuelve `header_md`/`body_md` además de `text`; (63.2) nueva `_split_long_para()` — el fallback degrada párrafo → líneas → palabras de una línea → corte duro, en vez de destruir todos los `\n`; (63.3) heading + caption en TODAS las partes de una tabla troceada, con presupuesto descontado antes de trocear; (62.1) el H1 del título y el preámbulo ya NO se clasifican (`level <= 1` → `"other"`) — antes contagiaban su etiqueta a las subsecciones sin clasificación propia vía el ascenso de ancestros; (62.2) `merge_empty_cover_blocks()` fusiona las portadas sin prosa propia con el chunk siguiente, heredando SU `section_canonical`, con guardas de "no hay siguiente" y "no pasarse de `MAX_EMBED_CHARS`", y renumerando `chunk_index`. Tests: `tests/test_chunking.py` (28). **Rollout ejecutado 2026-07-25** (ver Notas importantes): 29/1875 → 0/1859 chunks de tabla con filas fusionadas, 809/823 portadas fusionadas. | ✅ |
...
| `5_build_embeddings.py` | Genera índice FAISS con bge-m3 via Ollama. **Modo incremental por defecto**: solo embeddea papers nuevos (no en `indexed_papers.json`); `--force` para re-indexar todo desde cero. **2026-06-11**: escribe `year` en metadata.jsonl (denormalizado desde `papers_metadata.jsonl` + fallback regex); `embed_texts` con truncado reactivo ante exceso de contexto. **2026-07-25 (items 55a/55b + P2 Kimi)**: normaliza L2 los vectores antes de `index.add` con el helper único `utils.embeddings.l2_normalize` (el índice sigue siendo `IndexFlatL2`; sobre unitarios el orden L2 = coseno); `config.json` declara `"normalized": true`; invariante de integridad `verify_index_integrity()` al terminar (ntotal == metadata, ‖v‖ media ≈ 1, flag presente) que aborta con **SystemExit, no warning**; en modo incremental `assert_index_normalized()` comprueba el índice existente **antes de escribir nada** y aborta pidiendo `--force` si es pre-55a; persiste `embed_truncated: bool` por chunk. **Rollout ejecutado 2026-07-25**: 9/9 índices normalizados, `embed_truncated` en el 100% de 19.086 chunks. **Ojo (item 65): el modo incremental solo crece, nunca poda** — ver Notas importantes. | ✅ |
...
| `utils/retrieval.py` (NUEVO, item 33, 2026-06-11) | Funciones de recuperación: `dense_rank`, `bm25_rank`, `rrf_fuse` (RRF_K=60), `passes_filters` (centraliza filtros items 32/34), `pool_size`, `build_bm25`, `tokenize`. BM25 al vuelo desde metadata.jsonl, alineado con FAISS por orden. **2026-07-25 (item 55a):** `dense_rank` es el ÚNICO `index.search()` del repo y normaliza L2 la consulta ahí (sobre copia), cubriendo los 8 call sites densos de los 7 ficheros consumidores. **2026-07-25 (item 33 fase 1):** `tokenize` reescrito — NFKD para tildes, griegas conservadas, acrónimos con guion como token entero y por partes, tokens mixtos letra/dígito también por segmentos. No requiere reindexado. **2026-07-27:** `looks_spanish(text) -> bool` — heurístico sin dependencias (stopwords españolas distintas sobre los tokens plegados + presencia de ñ/tildes; ≥2 aciertos). Vive aquí y no en la UI porque es una propiedad de BM25: es léxico y el corpus está en inglés, así que una consulta en español solo casa cognados de IDF baja (*biogas*, *filter*, *pH*) y pierde los términos discriminantes, y `rrf_fuse` funde ese brazo casi aleatorio **a peso igual** con el denso. Solo alimenta el aviso de `2_RAG.py` / `7_Revision.py`; no bloquea ni altera el retrieval. | ✅ |
| `utils/embeddings.py` | Núcleo de embedding compartido (artículos + libros): `embed_texts` (uno a uno, truncado reactivo, `truncated_out` opcional → `embed_truncated`), `make_client`. **2026-07-25 (items 55a/55b):** ÚNICA implementación de la normalización — `l2_normalize()` (siempre devuelve copia float32 contigua), `index_mean_norm()`, `assert_index_normalized()` y `verify_index_integrity()`. Las usan los dos constructores de índice y las dos rutas de consulta; no duplicar en ningún otro sitio. | ✅ |
...
| `utils/download_registry.py` | Registro persistente de DOIs pendientes de descarga (pendientes_descarga.csv). **2026-07-12**: columna `snooze_until` (migración automática vía `load()` — CSVs antiguos sin la columna la rellenan a `""`) + `pending_active(df, today=None)` (fuente única de "qué es un pendiente activo": `status=="pending"` y snooze vacío o vencido, usada por `run_weekly_scopus.py` y por `15_Pendientes.py` para que no diverjan) + `snooze(dois, years=2, note="")` / `unsnooze(dois)`. `upsert()` no toca `snooze_until` en filas existentes (snooze-safe: la ingesta Scopus no des-pospone). **2026-07-12 (cont.):** `reconcile_with_corpus(categorias_dir=None)` — `mark_downloaded()` solo la llama `3a_download_pdfs.py`, así que un DOI ingestado por otra vía (PDF subido a mano) se quedaba `pending` para siempre aunque el paper ya existiera; esta función cruza los DOI `pending` contra `papers_metadata.jsonl` de todas las categorías y los marca `downloaded`. Corre automática en `15_Pendientes.py` y en `run_weekly_scopus.py`. Ejecutada contra el NAS real: 37 reconciliados (34 de 35 "pendientes activos" ya estaban en el corpus). **2026-07-27 — veto persistente:** `block(dois, reason="", rows=None)` (status `blocked`; **inserta fila nueva si el DOI no está en el registro** — un DOI que se descargó bien nunca pasa por `upsert()`, que solo registra fallos; `rows` rellena title/year/category en ese caso), `unblock(dois)` (vuelve a `pending`) y `blocked_dois() -> set[str]` (normalizado `.strip().lower()`: el CSV guarda el DOI tal cual llegó, así que **toda comparación va en minúsculas**). `"blocked"` añadido a `_RESOLVED` — sin eso el `upsert()` semanal pisaba el `reason` del veto con el error de descarga de turno. Como `pending_active()` filtra `status=="pending"`, un vetado nunca sale en el email semanal. **Fix 2026-07-27:** `block()` **no toca `snooze_until`** — la primera versión lo limpiaba, y como `unblock()` no sabe restaurarlo, vetar y reactivar un DOI pospuesto lo devolvía al email semanal de golpe. El `status='blocked'` ya excluye la fila de `pending_active()` sin necesidad de tocar el aplazamiento. | ✅ |
...
- `metadata.jsonl` por chunk incluye ahora `section_canonical` (item 32), `year` (item 34, denormalizado desde `papers_metadata.jsonl` + fallback regex sobre paper_id) y **`embed_truncated`** (bool, 2026-07-25, P2 auditoría Kimi): marca los chunks cuyo VECTOR representa solo parte de su `text`, por el recorte a `MAX_EMBED_CHARS` o por el truncado reactivo por contexto. `5_build_embeddings.py` imprime el recuento al terminar. No se persiste en libros (la caché `_vectors/*.npy` reutiliza vectores sin flags).
- **Cambios de comportamiento del chunker (items 62 y 63, 2026-07-25 — requieren re-trocear, ver nota de rollout):**
  - **El H1 (título del paper) y el preámbulo ya NO se clasifican** con `canonical_section()` (`level <= 1` → `"other"`). Antes un título con "Operational"/"characterization" etiquetaba el bloque de portada como methods/results **y**, por el ascenso de ancestros, contagiaba esa etiqueta a toda subsección del cuerpo sin clasificación propia. Consecuencia esperada tras el rollout: **el residuo `other` crece** — no es una regresión, es que antes había etiquetas falsas.
  - **Los bloques de portada sin contenido propio se FUSIONAN con el chunk siguiente** y heredan SU `section_canonical` (nunca el del remanente). Criterio de contenido, no posicional: hay línea `**Authors:**` y quedan < 200 chars de prosa tras quitar `**Authors:**`/`**Year|DOI**`. 809/823 casos en las 8 categorías. Guardas: sin chunk siguiente se emite tal cual; si el fusionado superaría `MAX_EMBED_CHARS` no se fusiona. `chunk_index` se renumera tras las fusiones. Los autores siguen siendo recuperables y no quedan vectores muertos diluyendo el pool.
  - **Cada fila de tabla es un párrafo** (`\n\n`, en las dos ramas de `extract_tables_md`), así que el splitter corta en frontera de fila y ya no puede fusionar filas de estudios distintos. El fallback de `_split_to_max_chars` degrada ahora por niveles (párrafo → líneas → palabras de una línea → corte duro) en vez de destruir todos los saltos. **Afecta también a los chunks de libros**: `books/process.py:132` reutiliza `_split_to_max_chars`.
  - **Todas las partes de una tabla troceada llevan `### Table N` + caption** (antes solo la parte 0; las siguientes quedaban como filas sueltas sin ancla semántica). El presupuesto se descuenta antes de trocear el cuerpo, así que ninguna parte se pasa de `MAX_EMBED_CHARS`.
- **Vectores L2-normalizados (item 55a, 2026-07-25):** bge-m3 vía Ollama devuelve vectores SIN normalizar (‖v‖ ≈ 25,8 de media medido sobre los 9 índices reales en el rollout; la cifra ‖v‖=27,11 citada originalmente el 2026-07-20 venía de un único vector suelto, no de una media — corregida aquí, no cambia la conclusión), así que la similitud de todo el corpus no era coseno. Ahora se normalizan en **un solo sitio por lado**, con el helper único `utils.embeddings.l2_normalize()` (siempre sobre COPIA — `faiss.normalize_L2` muta in-place y los callers reutilizan su `qv`): en los **dos** constructores de índice (`5_build_embeddings.py`, `books/embed.py`) y en las **dos** rutas de consulta (`utils.retrieval.dense_rank`, que es el ÚNICO `index.search()` del repo y cubre los 8 call sites densos; y `utils.attachments.rank_attachment`, que hacía L2 crudo en numpy y cuyos valores `fuse_results` mezcla POR DISTANCIA con los del corpus). **Se mantiene `IndexFlatL2`**: sobre vectores unitarios el orden L2 es el del coseno, así que ningún consumidor invierte su "menor = mejor". `config.json` declara `"normalized": true`.
- **⚠️ El modo incremental de `5_build_embeddings.py` SOLO CRECE, NUNCA PODA (item 65).** Sin `--force` solo añade `paper_id` nuevos: jamás comprueba si un `paper_id` que ya está en el índice ha desaparecido de `chunks/`, así que **cuarentenar un paper NO lo saca del RAG**. `quarantine_paper()`, `prune_orphan_metadata()` y `prune_orphan_tei()` limpian ficheros y `papers_metadata.jsonl`, pero no tocan el índice: sus chunks siguen compitiendo por el top-k hasta el siguiente `--force`. Caso real detectado en el rollout: `2019_santos_clotas_..._sewage_biogas_2`, cuarentenado el 2026-06-14, siguió aportando **20 chunks fantasma** al índice de anoxic durante **6 semanas**. Y ojo — **el invariante del 55b no lo detecta**: en incremental los vectores y `metadata.jsonl` se escriben juntos en append, así que `ntotal == len(meta)` sigue cuadrando (verde en los 9 índices con el fantasma dentro). Tras cualquier tanda de cuarentenas, lanzar `5_build_embeddings.py --force` de esa categoría. Detalle y fix propuesto en el item 65.
- **Invariante de integridad de índice (item 55b, 2026-07-25):** al terminar de construir, `verify_index_integrity()` aborta con **SystemExit (no warning)** si `index.ntotal != len(metadata.jsonl)`, si la norma L2 media ≠ 1,0 ± 1e-3, o si falta el flag `normalized` en `config.json`. En modo incremental se comprueba la norma del índice existente **antes de escribir nada**, así que un índice pre-55a aborta limpio pidiendo `--force` en vez de quedarse mezclado. Guard estructural en `tests/test_retrieval_normalization.py`: falla si aparece un `*index*.search(` fuera de `retrieval.py` o un tercer constructor de índice FAISS.
- `utils/retrieval.py` centraliza la recuperación: BM25 al vuelo (sin artefacto, alineado con FAISS por orden), fusión RRF (k=60) y filtros (`passes_filters`). Toggle híbrido OFF por defecto en CLI (`--hybrid`) y web. **Tokenizer arreglado 2026-07-25 (item 33 fase 1, no requiere reindexado):** tildes plegadas por NFKD, griegas conservadas, acrónimos con guion como token entero **y** por partes (`NR-SOB` → `nr-sob`, `nr`, `sob`), y tokens mixtos letra/dígito también por segmentos (`h2s` → `h2s`, `h`, `2`, `s`) — esto último es lo que hace que la consulta "H2S" case con el corpus, donde los subíndices vienen aplanados como "H 2 S". **Límite conocido de BM25 — no cruza idiomas (2026-07-27):** el corpus está en inglés y el brazo léxico no traduce, así que con la pregunta en español solo casan los cognados (`biogas`, `filter`, `pH`), que por frecuentes tienen IDF baja, mientras los términos discriminantes (`azufre`, `eliminación`, `lavador`) no están en ningún documento. El problema no es que BM25 "no ayude": es que el RRF lo funde **a peso igual** con el denso y puede empeorar el orden que daba el denso solo. El denso (bge-m3) sí es multilingüe. `looks_spanish()` detecta el caso y `2_RAG.py` / `7_Revision.py` avisan (sin bloquear) si el híbrido está ON. Es la hipótesis que bloquea la fase 2 del item 33.
...
- **✅ ROLLOUT items 62/63/55a/55b + `embed_truncated` COMPLETADO (2026-07-25, commit `6093f35`, ejecutado en pciq22).**
  **9/9 índices** normalizados (norma L2 media 1,000 y flag `"normalized": true` en los 9: las 8
  categorías + `libros_docencia`), invariante 55b en verde en todos. 809/823 portadas fusionadas
  (99,75%); chunks de tabla con filas fusionadas **29/1875 → 0/1859**; `embed_truncated` presente en
  el 100% de los 19.086 chunks (90 a `true`, 0,47%); `other` crece en 6/8 categorías. ~5h20min de
  re-embebido. Informe completo en
  `/Volumes/research/metadatos/eval/rollout_6093f35/informe.md`; resumen y números clave en
  `Mejoras_realizadas.md`. Efecto colateral: destapó el **item 65** (vectores fantasma).
  El procedimiento de abajo se conserva como **runbook para el próximo rollout de chunker**.
- **⚠️ TODO ROLLOUT DE CHUNKER/EMBEDDINGS ES ATÓMICO: LAS 8 CATEGORÍAS + EL ÍNDICE DE LIBROS, TODO O NADA.**
  Un rollout parcial rompe la fusión de **"Preparar clase"** (`14_Preparar_clase.py:138` ordena por
  distancia MEZCLANDO varios índices de categoría, y `:161` fusiona el de libros con ellos por
  distancia): un índice sin normalizar entre normalizados deja de competir y da resultados basura
  **sin avisar**. El flag `"normalized": true` de `config.json` existe precisamente para que ese
  estado sea detectable, pero no lo previene. No dejes el rollout a medias entre sesiones.
  1. **Papers, por cada una de las 8 categorías** (re-trocear + re-indexar):
     `python3 3_process_corpus.py --phase <cat> --force-md`  ← **`--force-md`, no `--force`**: es el
     que regenera `md_clean` + `chunks` **desde el TEI existente sin volver a llamar a GROBID**
     (`--force` sí re-invoca GROBID: horas de proceso y riesgo de TEI distinto por nada).
     Después: `python3 5_build_embeddings.py --project <cat> --phase all --force`  ← **`--force`
     obligatorio**: el modo incremental solo mira `paper_id` nuevos y no re-embebería los chunks
     regenerados de papers ya indexados. Además, sobre el índice viejo abortaría por el chequeo de
     norma del 55b (que es lo que se quiere: falla ruidoso, no mezcla).
  2. **Libros** (`libros_docencia`): necesita **re-trocear también**, no solo re-embeber —
     `books/process.py:132` reutiliza `_split_to_max_chars`, que cambió con el item 63.2. Desde
     `scripts/`: `python3 books/process.py --force` y luego `python3 books/embed.py --force`
     (ambos toman `--collection libros_docencia` y `--base /Volumes/research` por defecto).
     El pipeline de libros es solo CLI: no hay página de ingesta en Streamlit.
  3. **Antes de lanzar:** `.bak` de los índices; y comprobar si algún `golden_<cat>.jsonl` de
     `/Volumes/research/metadatos/eval/` referencia `chunk_id` (un re-chunk lo invalidaría).
  4. **Después:** re-correr `run_eval.py` en anoxic y upgrading **como alarma de regresión, NO como
     prueba de mejora** (n=4/n=6, ver item 37); y regenerar `corpus_manifest.json`.
  Efectos esperados y NO regresivos: el residuo `other` de `section_canonical` **crece** (había
  chunks etiquetados methods/results que nunca lo fueron, item 62); el número de chunks **baja**
  algo (fusión de 809 portadas vacías); las tablas grandes pasan a tener varias partes, todas con
  su `### Table N` + caption.
...
### Aprendizajes de método — medición y evaluación

- **Una métrica puede subir por un bug.** Tres casos en la misma semana (2026-07-25), los tres
  detectados mirando el MECANISMO, no la estadística:
  1. `test_accented_query_retrieves_unaccented_document` **pasaba con IDF = 0**: con exactamente 2
     documentos y el término en 1, la IDF de Okapi vale 0 exacto y el score sale 0 sea cual sea el
     tokenizer. El test daba verde sin comprobar nada.
  2. **Hit@8 = 1,0 en `biogas_upgrading_biomethanation`**, inflado por el sesgo de pooling con que se
     construyó el golden (los candidatos salieron del propio recuperador que luego se evalúa).
  3. **Hit@8 5/6 del híbrido de upgrading**, inflado por una **colisión de substring** entre español
     roto e inglés real: el tokenizer viejo partía `termófilos` en `term`+`filos` y `biometanación`
     en `biometanaci`+`n`, y esos restos casaban con "long-**term** storage" / "in **term**s of" del
     documento. El valor honesto era 4/6.
  Con n=4 y n=6 **nada de esto se detecta estadísticamente**. Regla: ante una métrica que sube o
  baja, preguntar POR QUÉ **antes** de actuar — la explicación mecánica es más barata que la
  potencia estadística que no se tiene.
- **Todo test end-to-end de BM25 necesita ≥3 documentos.** Con N=2 y un término presente en 1 solo
  documento, la IDF clásica de Okapi (la de `rank_bm25.BM25Okapi`) da
  `log((N − n + 0,5)/(n + 0,5)) = log((2 − 1 + 0,5)/(1 + 0,5)) = log(1) = 0` **exacto**, así que el
  score es 0 independientemente de si el tokenizer funciona o no. Es un caso degenerado del tamaño
  del corpus de prueba, no del código bajo test. Documentado como comentario en
