...
| `utils/retrieval.py` (NUEVO, item 33, 2026-06-11) | Funciones de recuperación: `dense_rank`, `bm25_rank`, `rrf_fuse` (RRF_K=60), `passes_filters` (centraliza filtros items 32/34), `pool_size`, `build_bm25`, `tokenize`. BM25 al vuelo desde metadata.jsonl, alineado con FAISS por orden. **2026-07-26 (item 55a):** `dense_rank` es el ÚNICO `index.search()` del repo y normaliza L2 la consulta ahí (sobre copia), cubriendo los 8 call sites densos de los 7 ficheros consumidores. **2026-07-26 (item 33 fase 1):** `tokenize` reescrito — NFKD para tildes, griegas conservadas, acrónimos con guion como token entero y por partes, tokens mixtos letra/dígito también por segmentos. No requiere reindexado. | ✅ |
| `utils/embeddings.py` | Núcleo de embedding compartido (artículos + libros): `embed_texts` (uno a uno, truncado reactivo, `truncated_out` opcional → `embed_truncated`), `make_client`. **2026-07-26 (items 55a/55b):** ÚNICA implementación de la normalización — `l2_normalize()` (siempre devuelve copia float32 contigua), `index_mean_norm()`, `assert_index_normalized()` y `verify_index_integrity()`. Las usan los dos constructores de índice y las dos rutas de consulta; no duplicar en ningún otro sitio. | ✅ |
...
- `metadata.jsonl` por chunk incluye ahora `section_canonical` (item 32), `year` (item 34, denormalizado desde `papers_metadata.jsonl` + fallback regex sobre paper_id) y **`embed_truncated`** (bool, 2026-07-26, P2 auditoría Kimi): marca los chunks cuyo VECTOR representa solo parte de su `text`, por el recorte a `MAX_EMBED_CHARS` o por el truncado reactivo por contexto. `5_build_embeddings.py` imprime el recuento al terminar. No se persiste en libros (la caché `_vectors/*.npy` reutiliza vectores sin flags).
- **Cambios de comportamiento del chunker (items 62 y 63, 2026-07-26 — requieren re-trocear, ver nota de rollout):**
...
- **⚠️ ROLLOUT PENDIENTE items 62/63/55a/55b + `embed_truncated` (código en main desde 2026-07-26) — ES ATÓMICO: LAS 8 CATEGORÍAS + EL ÍNDICE DE LIBROS, TODO O NADA.**
