# -*- coding: utf-8 -*-
"""
2descarga.py — Descarga de PDFs desde CSVs de Scopus/cribado por lotes.

Métodos de descarga:
  1) Semantic Scholar (OA, sin clave)
  2) Unpaywall
  3) Elsevier TDM API (requiere ELSEVIER_API_KEY en .env)
...
Caché: /Volumes/papers/04_cache/descarga_cache.json  (compartida entre lotes)
...
Requiere NAS montado en /Volumes/papers antes de ejecutar.

Estructura NAS:
  /Volumes/papers/
  ├── 00_inbox_scopus/     ← CSV exportado de Scopus (entrada por defecto)
  ├── 01_pdfs/auto/        ← PDFs descargados (plano, sin subcarpetas por año)
  ├── 02_logs/             ← logs por lote
  ├── 03_reports/
  │   ├── cribado/global/  ← CSVs de cribado (entrada típica de este script)
  │   └── descarga/        ← resultados por lote (XLSX, CSV, HTML, CSV pendientes)
  └── 04_cache/            ← caché compartida entre lotes

Ejemplos de uso:
  # a) dry-run con phase1A
  python 2descarga.py --csv /Volumes/papers/03_reports/cribado/global/cribado_global_phase1A.csv --dry-run

  # b) prueba con 10 artículos
  python 2descarga.py --csv .../cribado_global_phase1A.csv --max 10

  # c) ejecución completa con rutas explícitas
  python 2descarga.py \\
    --csv /Volumes/papers/03_reports/cribado/global/cribado_global_phase1B.csv \\
    --out-dir /Volumes/papers/01_pdfs/auto \\
    --cache /Volumes/papers/04_cache/descarga_cache.json \\
    --log /Volumes/papers/02_logs/descarga_phase1B.log \\
    --report /Volumes/papers/03_reports/descarga/resultado_descarga_phase1B.xlsx \\
    --report-csv /Volumes/papers/03_reports/descarga/resultado_descarga_phase1B.csv \\
    --pending-html /Volumes/papers/03_reports/descarga/pendientes_manual_phase1B.html \\
    --pending-csv /Volumes/papers/03_reports/descarga/pendientes_manual_phase1B.csv

  # d) rutas automáticas derivadas del stem del CSV (genera nombres _phase1B en /descarga/)
  python 2descarga.py \\
    --csv /Volumes/papers/03_reports/cribado/global/cribado_global_phase1B.csv \\
    --out-dir /Volumes/papers/01_pdfs/auto

  # e) timeout por artículo ajustado (75 s por defecto)
  python 2descarga.py --csv archivo.csv --max-seconds-per-article 75
  python 2descarga.py --csv archivo.csv --max-seconds-per-article 45

  # f) saltar artículos concretos por índice 1-based (van a pendientes manuales)
  python 2descarga.py --csv archivo.csv --skip-rows 15,181
  python 2descarga.py --csv archivo.csv --skip-rows 15,181,190-195
"""
...
NAS_ROOT = Path("/Volumes/papers")

# Subcarpetas que se crean automáticamente si el NAS está montado
_NAS_SUBDIRS = [
    NAS_ROOT / "00_inbox_scopus",
    NAS_ROOT / "01_pdfs" / "auto",
    NAS_ROOT / "01_pdfs" / "manual_import",        # TODO (fase 2): integración de PDFs manuales
    NAS_ROOT / "02_logs",
    NAS_ROOT / "03_reports",
    NAS_ROOT / "03_reports" / "manual_download",   # HTML y CSV de pendientes manuales
    NAS_ROOT / "03_reports" / "descarga",          # resultados por lote (XLSX, CSV, HTML)
    NAS_ROOT / "04_cache",
]


def check_nas_mounted() -> None:
    """Aborta con mensaje claro si /Volumes/papers no está accesible."""
    if not NAS_ROOT.exists():
        sys.exit(
            f"\nError: el NAS no está montado en {NAS_ROOT}\n"
            "Monta la unidad antes de ejecutar este script:\n"
            "  - Abre el Finder → Red → Synology → papers, o\n"
            "  - usa: open smb://synology/papers\n"
        )
...
@dataclass
class Config:
    csv_file: str = str(NAS_ROOT / "00_inbox_scopus" / "scopus_results.csv")
    output_dir: Path = field(default_factory=lambda: NAS_ROOT / "01_pdfs" / "auto")
    results_xlsx: str = str(NAS_ROOT / "03_reports" / "descarga" / "resultado_descarga_pdfs.xlsx")
    results_csv: str  = str(NAS_ROOT / "03_reports" / "descarga" / "resultado_descarga_pdfs.csv")
    log_file: Path = field(default_factory=lambda: NAS_ROOT / "02_logs" / "descarga.log")
    cache_file: str = str(NAS_ROOT / "04_cache" / "descarga_cache.json")
    cache_max_age_days: int = 30  # 0 = sin expiración
    # Pendientes manuales: HTML navegable + CSV plano, ambos en descarga/
    pending_html: str = str(NAS_ROOT / "03_reports" / "descarga" / "pendientes_manual.html")
    pending_csv: str  = str(NAS_ROOT / "03_reports" / "descarga" / "pendientes_manual.csv")
...
class DownloadManager:
    ...
    def save_results(self, results: List[DownloadResult], source_df: pd.DataFrame) -> None:
        ...
        #   Escanear /Volumes/papers/01_pdfs/manual_import/, emparejar PDFs con DOIs
...
def parse_args() -> argparse.Namespace:
    DESCARGA_DIR = NAS_ROOT / "03_reports" / "descarga"
    parser = argparse.ArgumentParser(
        description=(
            "Descarga PDFs académicos desde un CSV de Scopus/cribado por lotes. "
    ...
    parser.add_argument(
        "--csv", default=None,
        help=f"Ruta al CSV de entrada (por defecto {NAS_ROOT}/00_inbox_scopus/scopus_results.csv)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"Directorio de salida para los PDFs (por defecto {NAS_ROOT}/01_pdfs/auto)",
    )
    parser.add_argument(
    ...
    # --- Rutas de salida (opcionales; si se omiten se generan automáticamente desde el stem del CSV) ---
    parser.add_argument(
        "--report", default=None, metavar="XLSX",
        help=f"Ruta al Excel de resultados (por defecto {DESCARGA_DIR}/resultado_descarga_<stem>.xlsx)",
    )
    parser.add_argument(
        "--report-csv", default=None, metavar="CSV",
        help=f"Ruta al CSV de resultados (por defecto {DESCARGA_DIR}/resultado_descarga_<stem>.csv)",
    )
    parser.add_argument(
        "--log", default=None, metavar="LOG",
        help=f"Ruta al fichero de log (por defecto {NAS_ROOT}/02_logs/descarga_<stem>.log)",
    )
    parser.add_argument(
        "--cache", default=None, metavar="JSON",
        help=f"Ruta al fichero de caché (por defecto {NAS_ROOT}/04_cache/descarga_cache.json)",
    )
    parser.add_argument(
        "--pending-html", default=None, metavar="HTML",
        help=f"Ruta al HTML de pendientes (por defecto {DESCARGA_DIR}/pendientes_manual_<stem>.html)",
    )
    parser.add_argument(
        "--pending-csv", default=None, metavar="CSV",
        help=f"Ruta al CSV de pendientes (por defecto {DESCARGA_DIR}/pendientes_manual_<stem>.csv)",
    )
...
def main() -> None:
    ...
    csv_path = Path(args.csv) if args.csv else NAS_ROOT / "00_inbox_scopus" / "scopus_results.csv"
    stem = csv_path.stem  # e.g. "cribado_global_phase1B"
    DESCARGA_DIR = NAS_ROOT / "03_reports" / "descarga"

    cfg = Config(
        csv_file           = str(csv_path),
        output_dir         = Path(args.out_dir) if args.out_dir else NAS_ROOT / "01_pdfs" / "auto",
        results_xlsx       = args.report       or str(DESCARGA_DIR / f"resultado_descarga_{stem}.xlsx"),
        results_csv        = args.report_csv   or str(DESCARGA_DIR / f"resultado_descarga_{stem}.csv"),
        log_file           = Path(args.log)    if args.log    else NAS_ROOT / "02_logs" / f"descarga_{stem}.log",
        cache_file         = args.cache        or str(NAS_ROOT / "04_cache" / "descarga_cache.json"),
        pending_html       = args.pending_html or str(DESCARGA_DIR / f"pendientes_manual_{stem}.html"),
        pending_csv        = args.pending_csv  or str(DESCARGA_DIR / f"pendientes_manual_{stem}.csv"),
    ...
    # Crea subcarpetas del NAS que falten (no intenta crear NAS_ROOT si no está montado)
    ensure_nas_structure()

    if not cfg.unpaywall_email:
        sys.exit(
            "Error: falta UNPAYWALL_EMAIL en el fichero .env\n"
    ...
    if not Path(cfg.csv_file).exists():
        sys.exit(
            ...
            "Exporta tu búsqueda o ejecuta 0cribado.py primero "
            f"para generar el CSV en '{cfg.csv_file}'."
        )
