...
| `run_weekly_scopus.py` | Ingesta Scopus semanal autónoma. Ejecuta `run_scopus(WEEKLY_CATEGORIES, recent_days=7)` con `WEEKLY_CATEGORIES = ["biogas_upgrading_biomethanation", "anoxic_biogas_biodesulfurization"]`, cuenta PDFs nuevos y chunks, lee `pendientes_descarga.csv` y envía email HTML resumen via Gmail STARTTLS. Timeout 90 min via `ThreadPoolExecutor` + `future.result(timeout=5400)`; estado `"timeout"` si se supera y el email se envía igualmente. Logging a fichero (`logs/run_weekly_scopus_YYYY-MM-DD.log`) + stdout. Fallback HTML a `/tmp/`. `--dry-run` imprime HTML sin ejecutar. Config SMTP desde `.env`. | ✅ |
